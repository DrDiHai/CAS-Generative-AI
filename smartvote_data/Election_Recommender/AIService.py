...
class AIService:
    @staticmethod
    def get_recommendations(client: OpenAI, consolidated_candidates: str, max_retries: int = 5) -> dict:
        """
        ...
        while retries < max_retries:
            ...
            run = client.beta.threads.runs.create_and_poll(
                thread_id=recommend_thread.id,
                assistant_id='asst_Ae7oWeGm396Uo92X1TUxFoLb',
            )
            ...
            recommend_messages = list(client.beta.threads.messages.list(
                thread_id=recommend_thread.id
            ))
            ...
            try:
                data = json.loads(extracted_json) if extracted_json else json.loads(response_text)
        ...
        raise ValueError("Failed to get valid AI recommendations after multiple attempts.")
