...
class Candidate:
    def __init__(self, db: DBConnection, client:OpenAI, ID_candidate:int):
        ...
        data = self.db.execute("""SELECT Kandidat.*, 
        ...
        WHERE Kandidat.ID = ?""", [ID_candidate])
        if not data:
    ...
    def create_profile(self, base_data, **kwargs):
        candidate_profile = '\n'.join([f"{item}: {base_data[item]}" for item in base_data if base_data[item]])
        cleavages = self.fetch_candidate_answers(self.candidate_id, [AnswerType.CLEAVAGE])
        if cleavages:
    ...
    def __repr__(self):
        return (f"<Candidate(id={self.candidate_id}, election_id={self.election_id}, "
    ...
    def summary(self, **kwargs):
        ...
        summary_data = self.db.execute("SELECT Summary FROM Candidate_Summary WHERE Candidate_ID = ? AND Election_ID = ? AND Variant_Key = ?",
                                  (self.candidate_id, self.ID_election, variant_key))
        
        # Return existing summary if found
        if summary_data:
            print(f"Summary found for candidate {self.candidate_id} in election {self.ID_election}, variant {kwargs}")
            return summary_data[0]["Summary"]
        
        # Otherwise, create a new summary, save it, and return it
        print(f"Creating new summary for candidate {self.candidate_id} in election {self.ID_election}, variant: {kwargs}")

        new_summary = self.base_profile(**kwargs) if kwargs.get("use_base_profile", False) else self.create_ai_summary(**kwargs)

        print(f"New summary created: {new_summary}, saving to database")
        self.db.execute("INSERT INTO Candidate_Summary (Candidate_ID, Election_ID, Variant_Key, Summary) VALUES (?, ?, ?, ?)",
    ...
    def create_ai_summary(self, **kwargs):
        ...
        message = self.client.beta.threads.messages.create(
            thread_id=thread.id,
            role="user",
            content=self.base_profile(**kwargs)
        )
    ...
    def fetch_candidate_answers(self, candidate_id, answer_types:list[AnswerType]):
        ...
        for row in results:
            answer_id, dimension, question_text, answer_text, value, answer_type, comment = row
            
            if dimension not in answers_by_dimension:
                answers_by_dimension[dimension] = {'question': question_text, 'answer': '', 'comment': '', 'cleavage_value': ''}
        ...
        for dimension, entry in answers_by_dimension.items():
            ...
            if AnswerType.CLEAVAGE in answer_types and entry['cleavage_value']:
                full_answer = ", ".join([cur for cur in [full_answer, entry['cleavage_value']] if cur])
            if AnswerType.ANSWER in answer_types and entry['answer']:
                full_answer = ", ".join([cur for cur in [full_answer, entry['answer']] if cur])
            if AnswerType.COMMENT in answer_types and entry['comment']:
                full_answer = ", ".join([cur for cur in [full_answer, entry['comment']] if cur])
